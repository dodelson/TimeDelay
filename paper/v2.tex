...
&& + \sqrt{L(L+1)} d_{LM} E^{\alpha}_{L}
\eea
where $E^{\alpha}_{L}$ is given by
\be
E^{\alpha}_{L} = \frac{\sum_{l_{1}l_{2}}h_{l_{1}l_{2}}^{\alpha,\phi}(L)g_{l_{1}Ll_{2}}^{\alpha}}{\sum_{l_{1}l_{2}}h_{l_{1}l_{2}}^{\alpha,\phi}(L)f_{l_{1}Ll_{2}}^{\alpha}}
\ee
Now we are mostly interested in $L\approx 30$ because that when lensing signal is at maximum. $E^{\Theta \Theta}_{30} \approx 0.00157$ so the contribution from time delay is actually negligible (for $\Theta \Theta $ case).
