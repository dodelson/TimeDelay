...
\eea
 ...
 should be corrected for this effect at the lowest $L$. \peikai{Actually $\frac{(F_1^{*}+F_1^{} )C_1^{\phi d}}{\sqrt{C_1^{\phi\phi} C_1^{dd} }}
  + F_1^{*}F_1^{}\approx0.22$ which is the multiplicative bias for $\langle \phi \phi \rangle$ induced by time delay contamination at $L=1$.While }in the regime relevant to current 
