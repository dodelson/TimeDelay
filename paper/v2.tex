...
\Spng{StoN}{Time Delay Signal to Noise, with the formula $\frac{S}{N}=\sum_{L}\left\{ \frac{C^{dd}_{L}}{\sqrt{\frac{2}{(2L+1)f_{\rm sky}}}(C^{dd}_{L}+B^{\alpha}_{L})}\right\}^{2}$ for different estimators. Here we choose $f_{\rm sky}=1$.}
Here we keep $l_{\rm min}$ fixed at $1000$ and let $l_{\rm max}$ vary (note that the choice of $l_{\rm min}$ will only have a very little influence, e.g. if we set $l_{\rm min}=500$, $S/N$ will only have $\approx 1\%$ difference when $l_{\rm max}$ is fixed at $4000$). We can see that at $l_{\rm max} \approx 5000$, minimum variance estimator $S/N \approx 1$.  \wh{some words on why $l_{\rm min}$? and whether that matters?}
