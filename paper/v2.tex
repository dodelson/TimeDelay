...
$\langle \hat d _{LM} \rangle = d_{LM}$ when $\phi_{LM}=0$.  We return to the issue
of cross contamination of the estimators in Sec.~\ref{sec:cross}.}
...
An estimator can now be constructed for each of the pairs of fields (except $BB$ \wh{what does this refer to given the table has BB - do you mean that we later assume the undistorted BB, $\tilde C_l^{BB}=0$} \peikai{I actually computed $\tilde{C}^{BB}$ and $\tilde{C}^{BB,d}$ using tensor mode; while for $\Theta$ and E I used scalar modes only. So could we just set $a=\Theta,E,B$ to make it consistent? Maybe make an annotation for it.}), so letting $\alpha$ denote pairs of fields $(ab)$, we have
\bea
...
We can see that at $l_{\rm max} \approx 5000$, the minimum variance estimator detectability reaches 1, and at 7000 is 1.42 but of course this is for the most optimistic of configurations. \wh{%What noise level is this?  if no noise why isn't $\Theta\Theta$ above one at $l_{\rm max}=7000$ as it seems to be in previous plot?  
