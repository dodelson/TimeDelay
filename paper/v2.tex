...
%%%%% FIGURE %%%%%%%
\Sfig{kernel}{Kernel that weights the integral of the gravitational potential for the time delay examined here and the more carefully studied deflection angle. \wh{Flip solid and dashed  if the latter means negative sign?  also keeping a consistent color scheme between the two figures would help e.g. red for $\phi$, blue for $d$, purple for cross}}
%%%%% FIGURE %%%%%%%
...
\wh{Note from Figure 2 that $C^{\phi d}_{L}$ is negative and comparable to the auto spectra and so the two fields $\phi$ and $d$  are highly anticorrelated. }
...
\eea 
where the superscript $c$ stands for cross. The noise in the estimators is correlated because 
