...
from gravitational waves. \peikai{We should also notice the emission angle effect, which is discussed in \cite{Lewis:2017ea}, is the same order as the time delay effect\footnote{\peikai{There is a module in recent versions of CAMB which is able to calculate the corrections to BB power spectrum due to time delay and emission angle effects. See \url{https://camb.readthedocs.io/en/latest/emission_angle.html}.}}. There can be significant cancellations between $B$ modes of these two effects. Since $B$ modes only have a little impact on time delay detectability, we leave the detailed calculation to future work.}
...
\peikai{There's been methods such as bias-hardened method developed in CMB lensing case to remove biases from full sky reconstruction, see \cite{Namikawa:2014bh} for detailed discussion. Here we only focus on the magnitude of cross contamination on low $L$.} We can then compute minimum variance estimator contaminations using \ec{dmvw} as
\bea
E_{L} &=&  
