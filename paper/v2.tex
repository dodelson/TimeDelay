...
\Sfig{StoN}{Detectability of the spectrum of the time delay signal, $\sigma_d^{-1}$, using \ec{sn} for different quadratic estimators as a function of the largest $l$-mode accessed. Here we set $f_{\rm sky}$ to be $1$.}
...
% \peikai{One question, how do we construct mv estimators here? To build a full mv estimator like before, like what Eq.(31-33) did, we need $N^{\alpha \beta}$ where $\alpha$ don't need to be equal to $\beta$. But here we only have $\alpha=\beta$ case.}
 ...
 at $L=1$.  \peikai{$F_{L=1} \approx 0.24$  while $F_{L=2} \approx 0.05$, we can see there's a rapid decrease.}
...
\Sfig{Cont_multi}{\peikai{Cross contamination. Notice that $E$ and $F$ are all negative. $\frac{(F_1^{*}+F_1^{} )C_1^{\phi d}}{\sqrt{C_1^{\phi\phi} C_1^{dd} }}
