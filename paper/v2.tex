...
from gravitational waves. We also note that the emission angle effect, which is discussed in \cite{Lewis:2017ea}, is the same order of magnitude as the time delay effect.\footnote{There is a module in recent versions of CAMB that is able to calculate the corrections to the BB power spectrum due to time delay and emission angle effects. See \url{https://camb.readthedocs.io/en/latest/emission_angle.html}.} There can be significant cancellations between these two effects in the generation of $B$ modes. Since $B$ modes have little impact on the time delay detectability, we neglect the emission angle effect here.
...
the estimators. Alternately bias-hardening \cite{Namikawa:2014bh} can also be applied to remove bias at the expense of additional variance. 
