...
\eea 
\wh{clarify sign of cross term.  sanity test on overdensity being with the new conventions: positive $\Phi$, positive $\phi$ and negative $d$ - so negative?
 should have flipped from the old kernels so was it positive before?}
where the superscript $c$ stands for cross.  Note that the noise in the estimators is correlated because 
...
\eea
 ...
 Conversely, the relative contamination $F$ for $\phi$ decreases with $L$: $F_{L=1} \approx 0.24$,  $F_{L=2} \approx 0.05$ \wh{caption says $F$ is negative?}.  In principle lensing estimators 
