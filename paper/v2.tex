...
from gravitational waves. \peikai{We also note that the emission angle effect, which is discussed in \cite{Lewis:2017ea}, is the same order of magnitude as the time delay effect\footnote{\peikai{There is a module in recent versions of CAMB that is able to calculate the corrections to the BB power spectrum due to time delay and emission angle effects. See \url{https://camb.readthedocs.io/en/latest/emission_angle.html}.}} \wh{and tend to cancel in their impact on  $B$ modes}. Since $B$ modes themselves have little impact on the time delay detectability, we neglect the emission angle effect here.}
...
the estimators.   \wh{Alternately bias-hardening \cite{Namikawa:2014bh} can also be applied to remove bias at the expense of additional variance. } 
