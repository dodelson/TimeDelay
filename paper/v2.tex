...
We saw in Fig.~\rf{Delay} that temperature maps only are not sufficient to detect this signal. To assess the added information contained in the polarization field, we show the signal to noise as a function $l_{\rm max}$ for a noiseless experiment in Fig.~\rf{StoN}.
...
\Spng{StoN}{Time Delay Signal to Noise with $L=1$,$l_{min}$ fixed at $1000$, $l_{max}$ varying}
