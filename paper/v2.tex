...
normalization?}\scott{seems to agree with Cooray and Hu, at least at l=1, where it's easiest to check}}
...
\bea
\omega^{\alpha}(L) & =&N^{d,\rm mv}_{L} \sum_{\beta}(N_{L}^{d,-1})^{\alpha \beta} \eql{weight}\\
...
Weighting the quadratic pair with the $w^\alpha$'s for $d$ as in \ec{weight} and the analogous weighting for $\phi$ leads to noise on the product of this pair of minimum variance estimators $\hat d^{\rm mv}\hat\phi^{*,{\rm mv}}$ equal to
\be
N^{\rm c,mv}_L = \sum_{\alpha\beta } w^{\alpha,d}_L w^{\beta,\phi}_L N^{\rm c,\alpha\beta}_L
\ee
...
&\times&\frac{(C_L^{\phi d})^2}{( C_L^{\phi d}+N_{L}^{\rm c,mv})^2+
(C_L^{\phi\phi}+N_L^{\phi, mv})(C_L^{dd}+N_L^{\rm d,mv})}. \nonumber
\eea
