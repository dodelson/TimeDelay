...
\be
C^{\Theta\Theta,d}_{l} \equiv \frac{2}{\pi}\,\int_0^\infty \frac{dk}{k} \, (S(k;\eta_*))^2\,j_l(kD_*)\,\frac{\partial j_l(kD_*)}{\partial\ln D_*}\eql{cld}
\ee
...
&&\rightarrow
\int d\eta g(\eta) S(k;\eta) j_l(kD(\eta)) \vs &&\times \int d\eta' g(\eta') S(k;\eta') \frac{\partial j_l(kD(\eta'))}{\partial\ln D(\eta')}\eql{timeevol}
\eea
with $g(\eta)$ the visibility function, sharply peaked at $\eta=\eta_*$.\scott{Wayne, is this what you meant? Both: is this right?}
\peikai{There is acually a slowly varying term $\mathcal{P}_{R}(k)$ in the integrand, which is the primitive power spectrum in CAMB code, but if I just add it here, it seems to be inconsistent with previous derivation.}
...
\bea
C^{\Theta E,d}_{l} &\equiv& \frac{2}{\pi}\,\int_0^\infty \frac{dk}{k}\, \int d\eta\,g(\eta)S(k;\eta) j_{l}(kD(\eta)) \vs
&&\times \int d\eta' g(\eta') S^{E}(k,\eta')\,\frac{\partial j_l(kD(\eta'))}{\partial\ln D(\eta')}
\eea
with $S^{E}(k;\eta)$ is the source functon of the E-mode of the polarization field; defined in CAMB and we only include scalar mode here, see Ref.~\cite{Hu:1997de} for a detailed description.
\peikai{Not sure if the description here is detailed enough...}
...
We can see that at $l_{\rm max} \approx 5000$, the minimum variance estimator detectability reaches 1, but of course this is for the most optimistic of configurations. \wh{%What noise level is this?  if no noise why isn't $\Theta\Theta$ above one at $l_{\rm max}=7000$ as it seems to be in previous plot?  
...
\bea
\mathcal{N}_{L}^{\alpha\beta}&=&\frac{\bar{A}_{L}^{\alpha*}\bar{B}_{L}^{\beta}}{(2L+1)}\sum_{l_{1}l_{2}}  \left\{ \bar{h}_{l_{1}l_{2}}^{\alpha,\phi*} (L)\big[ C_{l_{1}}^{ac}C_{l_{2}}^{bd}\bar{h}_{l_{1}l_{2}}^{\alpha,d}(L)\right. \nonumber \\
...
with 
\bea 
\bar{A}_{L}^{\alpha} &=& (2L+1)\left\{ \sum_{l_{1}l_{2}}\bar{h}_{l_{1}l_{2}}^{\alpha,\phi}(L)f_{l_{1}Ll_{2}}^{\alpha} \right\}^{-1} \nonumber \\
&=& (2L+1)\left\{ \sum_{l_{1}l_{2}}h_{l_{1}l_{2}}^{\alpha,d}(L)f_{l_{1}Ll_{2}}^{\alpha} \right\}^{-1} \\
...
In this case
\bea
\mathcal{N}_{L}^{\alpha \alpha} &=& \bar{A}_{L}^{\alpha}=\bar{B}_{L}^{\alpha}  \nonumber \\
&=& (2L+1)\left\{ \sum_{l_{1}l_{2}}h_{l_{1}l_{2}}^{\alpha,d}(L)f_{l_{1}Ll_{2}}^{\alpha} \right\}^{-1} \nonumber \\
&=& (2L+1)\left\{ \sum_{l_{1}l_{2}}h_{l_{1}l_{2}}^{\alpha,\phi}(L)g_{l_{1}Ll_{2}}^{\alpha} \right\}^{-1}
\eea
