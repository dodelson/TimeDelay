...
The theory of general relativity dictates that particles traveling through gravitational potential wells experience time delays~\cite{1964PhRvL..13..789S}. If two photons are emitted at the same time, then they will travel different distances depending upon the potential $\Phi$ through which they travel. In the cosmological context of an expanding, Euclidean universe
\wh{what does Euclidean mean here?  spatially flat background?}
...
This directional-dependent change in the distance to last scattering is independent of its finite width and is a phenomenon difference \wh{something not parsing here} from the angular deflections~\cite{Hu:2001tn,Lewis:2006fu} that have been captured by recent experiments~\cite{Smith:2007rg,Ade:2013tyw,Story:2014hni,Sherwin:2016tyf,Aghanim:2018oex}.
...
\ee
with the first term on the right the signal and the second the noise. Fig.~\rf{Delay} shows the signal and noise at each $ L$ for several experimental configurations. Since the signal can be extracted from all available $d_{LM}$, the total signal to noise for a measurement is
\be
\left(\frac{S}{N}\right)^2 = \sum_L^{\infty} \frac{(2L+1)f_{\rm sky}}{2} (\frac{C_L^{dd}}{C_L^{dd}+B_L})^2\eql{sn}
.\ee
\wh{This doesn't look right to me at least the mode counting is inverted but I also don't understand what S/N this is
 - I suggest you define the signal to noise as the number of sigma that you an detect the given power spectrum as in my note.  Also, the connection with what is plotted in the figure
 isn't made explicit here.}
We see that most of the signal arises from the lowest $L$-modes, particularly $L=1$. However, even for a full-sky experiment, the most optimistic niose projections lead to a total signal to noise less than 1, using temperature only.
...
.\eea
The functions $f^\alpha$ and $g^\alpha$ are the generalizations of \ec{lfg} to include polarization (\ec{lfg} now corresponds to $\alpha=\Theta\Theta$). The full set of $f^{\alpha}$ was determined by Ref.~\cite{Okamoto:2003zw} and is reproduced in Table 1, which now includes the full set of $g^{\alpha}$ that govern the impact of changing radial distances. Note that $\tilde C_l$ corresponds to the spectra of the unlensed fields.  \scott{I think we should change the $\tilde C \rightarrow C$ here, since they are not the spectra of the unlensed fields: they are the radial derivatives of the spectra, right?} \wh{They are radial derivatives of the unlensed (i.e. before lensing) fields}
 \scott{Also, propose to add the following few lines, replacing the sentence above that starts ``Note that ...''}
\scott{\bf Note that the angular deflection coefficients $f^\alpha$ included the unlensed field because the transverse derivatives are captured by powers of $\ell$. The radial derivatives must be computed directly, as in \ec{cld}. For example,
\be
C^{\Theta E,d}_{l} \equiv \frac{2}{\pi}\,\int_0^\infty dk\,k^2\, P^{\Theta E}(k,\eta_*)\,j_l(kD_*)\,\frac{\partial j_l(kD_*)}{\partial\ln D_*}
\ee
with $P^{\Theta E}(k,\eta_*)$ the 3D cross-power spectrum at the time of recombination of the temperature field and the E-mode of the polarization field.}
...
We saw in Fig.~\rf{Delay} that small scale temperature maps only are not sufficient to detect this signal. To assess the added information contained in the polarization field, we show the signal to noise for the lowest $L$-mode as a function $l_{\rm max}$ for a noiseless experiment in Fig.~\rf{StoN}.
...
\Sfig{StoN}{Time Delay Signal to Noise, using \ec{sn} for different estimators. Here we set $f_{\rm sky}$ to be $1$.}
...
\eea
When $(\alpha\beta)=(\gamma \sigma)$, this simplified to
\bea 
...
\eea
\noindent And we can compute cross power spectrum signal-to-noise by assuming that $(C_{L}^{\phi d}+\mathcal{N}_{L}^{\alpha \beta})=0$ and use minimu variance estimator for both lensing and time delay. Signal-to-noise in FIG.5 is given by:
\be
(\frac{S}{N})^{2} = \sum_L^{\infty} (2L+1)f_{\rm sky} \frac{C_L^{\phi d}}{C_L^{\phi \phi}+N_L^{\phi,\rm mv}}\frac{C_L^{\phi d}}{C_L^{dd}+N_L^{d,\rm mv}}\eql{sn}
.\ee
We set $f_{\rm sky}=1$ and sum up tp $L=100$. We could see that in this ideal case, $S/N$ could reach about $2.5$, which still may not be detectable.
\Sfig{CrossStoN}{Cross Power Spectrum S/N \peikai{Signal to Noise looks really bad for this one...}}
