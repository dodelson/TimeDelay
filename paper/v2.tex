...
\bea
\tilde{C}^{\Theta E,d}_{l} \equiv \frac{2}{\pi}\,\int_0^\infty k^{2}dk \,&&\mathcal{P}_{R}(k)\left\{T_{\Theta}[j_l;k]T_{E}^{(1)}[j_l^{(22)};k]\right. \vs
&& \left.+ T^{(1)}_{\Theta}[j_l;k]T_{E}[j_l^{(22)};k] \right\}/2
\eea
similarly with 
\begin{equation}
T_E^{(1)} \equiv \frac{\partial T_E}{\partial \ln D}
\end{equation}
differentiation over radial function $j_{l}^{(22)}(x)\propto j_l(x)/x^2$ inside the integrand, notice we only include scalar mode here. See Ref.~\cite{Hu:1997de} for a more detailed discussion.
