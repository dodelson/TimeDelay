...
to recombination.  We shall see below that we an express this radial derivative in
terms of the radiation transfer function.
...
Note the difference between these two functions. \wh{something probably got lost in the shuffle, which two functions?}  Each involves a derivative. The one that governs deflections, $f$, %depends on the unlensed CMB spectrum $\tilde{C}_{l}^{\Theta\Theta}$
...
\wh{labels on all figures should be larger}}
...
that
\bea
\tilde{C}^{XY}_{l} \equiv \frac{2}{\pi}\,\int_0^\infty k^{2}dk \,&&{P}_{R}(k) T_{X}[l;k]T_{Y}[l;k]\eea
denotes the power spectra of the undistorted fields with $T_X$ as the radiation transfer
function for the field $X$ and \bea
...
As a first attempt, we consider the cross correlation of the two estimators
\bea
 \langle \hat{\phi}^{\alpha*}_{LM}\hat{d}^{\beta}_{LM} \rangle= \hat{C}_{L}^{\phi d} +N_{L}^{c},
\eea 
