...
\eea
where $a$ is the scale factor. In a cosmological context of an expanding universe, the %fractional difference in comoving distance $D$ to a source at redshift $z$ is equal to  
%\be
...
\wh{$\theta$ hasn't been defined and it's not clear how a time delay is being compared to an angle anyway - maybe: the
fractional time delay is much smaller, of order the lensing deflection angles...}
...
.\eea
Here, we have written the integral over the product of three spherical harmonics as $_0I$ and $_0J$ to enable generalization to the case of polarization \wh{which involve spin $s=2$ harmonics}. The general expression is  
...
involves a derivative with respect to the transverse directions so $F$ as defined in \ec{fg} has more powers of $l$ than does $G$.% (which are derivatives with respect to transverse position in real space) contained in $F$ compared to $G$ in \ec{fg}. 
...
Here we keep $l_{\rm min}$ fixed at $1000$ (we tested that our final results are insensitive to this choice) and let $l_{\rm max}$ vary %(note that the choice of $l_{\rm min}$ will only have a very little influence, e.g. if we set $l_{\rm min}=500$, $S/N$ will only have $\approx 1\%$ difference from when $l_{\rm max}$ is fixed at $4000$). 
We can see that at $l_{\rm max} \approx 5000$, the minimum variance estimator signal to noise reaches 1, but of course this is for the most optimistic of configurations. %$S/N \approx 1$.  \wh{some words on why $l_{\rm min}$? and whether that matters?}
