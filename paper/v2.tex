...
we denote the noise of the minimum variance combination with no superscripts for simplicity.
...
As a first attempt, we consider the cross correlation of the $d$-field with the $\phi$-field responsible for deflections. Without further optimization, the cross-spectrum for $\alpha=ab$ and
...
We can also construct the analogous noise cross spectrum for the minimum variance weighted estimators for $\phi$ and $d$ separately using
the weights of \ec{weight} 
%
%Weighting the quadratic pair with the $\omega^\alpha$'s for $d$ as in \ec{weight} and the analogous weighting for $\phi$ leads to noise on the product of this pair of minimum variance %estimators $\langle \hat\phi^{{\rm mv}*}_{LM} \hat d^{\rm mv}_{LM}\rangle$ equal to
\be
...
For the minimum variance combinations of the individual estimators we can again write
%\bea
