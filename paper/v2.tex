...
normalization?}\peikai{absolute normalization }\scott{seems to agree with Hu and Cooray, at least at l=1, where it's easiest to check}}
...
\bea
\omega^{d,\alpha}(L) & =&N^{d,\rm mv}_{L} \sum_{\beta}(N_{L}^{d,-1})^{\alpha \beta} \eql{weight}\\
N^{d,\rm mv}_{L} &=& \frac{1}{\sum_{\alpha\beta} (N_{L}^{d,-1})^{\alpha \beta}}
\eea
where $N_{L}^{d,-1}$ is the inverse matrix of time delay noise given by \ec{full}, with matrix indices given by polarizations. 
...
Weighting the quadratic pair with the $\omega^\alpha$'s for $d$ as in \ec{weight} and the analogous weighting for $\phi$ leads to noise on the product of this pair of minimum variance estimators $\hat\phi^{{\rm mv}*}\hat d^{\rm mv}$ equal to
\be
N^{\rm c,mv}_L = \sum_{\alpha\beta } \omega^{d,\alpha}(L) \omega^{\phi,\beta}(L) N^{c,\alpha\beta}_L
\ee
...
(C_L^{\phi\phi}+N_L^{\phi, \rm mv})(C_L^{dd}+N_L^{d,\rm mv})}. \nonumber
\eea
...
We can then compute minimum variance estimator contamination using \ec{dmvw} and get
\bea
E^{\rm mv}_{L} &=&
\sum_{\alpha}\left\{\omega^{d,\alpha}(L)\frac{N_{L}^{c,\alpha\alpha}}{N_{L}^{\phi,\alpha\alpha}}\right\} \sqrt{\frac{C_{L}^{\phi\phi}}{C_{L}^{dd}}},\nonumber\\
F^{\rm mv}_{L} &=&\sum_{\alpha}\left\{\omega^{\phi,\alpha}(L) \frac{N_{L}^{c,\alpha\alpha}}{N_{L}^{d,\alpha\alpha}}\right\}\sqrt{\frac{C_{L}^{dd}}{C_{L}^{\phi\phi}}}
\eea
%\wh{for individual $\alpha$ are you using the form with $N_L$?  not sure how it simplifies
%ike that -- did you assume $\alpha=aa$, two of the same fields or does that equality hold for
%all $\alpha$?}
...
% \peikai{One question, how do we construct mv estimators here? To build a full mv estimator like before, like what Eq.(31-33) did, we need $N^{\alpha \beta}$ where $\alpha$ don't need to be equal to $\beta$. But here we only have $\alpha=\beta$ case.}
 ...
 Conversely, the relative contamination $F$ for $\phi$ decreases with $L$ and is most signficant
 ...
 \bea
  ...
  \left[ 1 +\frac{(F_L^{\rm mv*}+F_L^{\rm mv} )C_L^{\phi d}}{\sqrt{C_L^{\phi\phi} C_L^{dd} }}
  + F_L^{\rm mv*}F_L^{\rm mv} \right] C_L^{\phi\phi} 
  \nonumber\\
  &&+ N_L^{\phi,\rm mv} \nonumber\\
    \langle \hat{d}^{\rm mv*}_{LM}\hat{d}^{\rm mv}_{LM} \rangle &=&
  \left[ 1 +\frac{(E_L^{\rm mv*}+E_L^{\rm mv} )C_L^{\phi d}}{\sqrt{C_L^{\phi\phi} C_L^{dd} }}
  + E_L^{\rm mv*}E_L^{\rm mv} \right] C_L^{dd} 
  \nonumber\\
  &&+ N_L^{d,\rm mv} \nonumber\\
 \langle \hat{\phi}^{\rm mv*}_{LM}\hat{d}^{\rm mv}_{LM} \rangle&=& (1+ F_L^{\rm mv*} E_L^{\rm mv}){C}_{L}^{\phi d}+ \sqrt{C_{L}^{\phi\phi} C_{L}^{dd}} ( F_L^{\rm mv*} + E_L^{\rm mv}) \nonumber\\
&&  +N_{L}^{c,\rm mv} 
