...
\ee
where $\tau$ is the optical depth, which becomes very large at times smaller than the epoch of  last scattering, $t_*$ or equivalently when $z>z_*$. \wh{In the kernel $D(z) \rightarrow \int_0^\infty e^{-\tau(z') }dz'/H(z')$?  should the
kernel just be introduced here without a $z'$, i.e. define $K(z',D_*)$ with this definition for $D_*$, leaving the first equation as you originally had it in $dD$ space as the
intuitive start point?}
...
Although deflections and delays are two different phenomena, they share some similarities, especially in the case of the CMB. Both are determined by the integrated potential along the line of sight, although with slightly different kernels,
\wh{perhaps give explicitly since this one that might confuse people who are thinking about convergence (and that includes me, since I would expect this kernel if defined for projected potential $\phi$ and not $\nabla^2\phi$ to diverge relative to $K$ as 
$z'\rightarrow 0$?  i.e.
with the relationship between that of the delay and the deflection potential $\phi$ as
\be
K_\phi(z',D_*) \approx K(z',D_*)  \frac{D(z')-D_*}{ D(z') } 
\ee
for $D(z')<D_*$.
}
 as depicted in Figure~\rf{kernel}. It is clear that they will be highly correlated, so as a first approximation, we might view the maps of the lensing potential created for example in \citet{Aghanim:2018oex} as maps of distance to the last scattering surface. Another similarity, one that has not yet been exploited, is that the formalism first proposed in \citet{Hu:2001tn} can be applied to the delays as well, and this is what we will do in this paper. We start though with the rather humbling calculation of \citet{Hu:2001yq} that the RMS fractional distance differences are a factor of ten smaller than the RMS angular deviations. Further, while the latter peaks at degree scales, the former peak on the largest scales.
